...
from rdkit.Chem import BRICS, MolFromSmiles, MolFragmentToSmiles
from collections import defaultdict
from rdkit import Chem
import re
_BRICS_LABEL_RE = re.compile(r'\[(\d+)\*\]')
def get_brics_fragments(smiles):
    ...
    if mol is None:                                   
        return []                                     
    frag_set = BRICS.BRICSDecompose(mol, minFragmentSize=1)  
    return list(frag_set) if frag_set else []         

def count_frag_freq(smiles_list):
...
def brics_labels(frag_smiles):
    return [int(x) for x in _BRICS_LABEL_RE.findall(frag_smiles)]


DEFAULT_LABEL_WEIGHTS = {i: 1.0 for i in range(1, 17)}

def fragment_conn_priority(frag_smiles, label_weights=DEFAULT_LABEL_WEIGHTS):
    labels = brics_labels(frag_smiles)                 
    if not labels:                                     
        return 0.0                                    
    return float(                                      
        sum(label_weights.get(l, 1.0) for l in labels) 
    )
...
def build_fragment_importance(high_mols, low_mols, w_conn=0.3, w_enrich=0.7,
                              label_weights=DEFAULT_LABEL_WEIGHTS, alpha=1.0):

    freq_high = defaultdict(int)  
    freq_low = defaultdict(int)   
    conn_priority = {}            

    for m in high_mols:                                   
        for f in set(get_brics_fragments(m)):             
            freq_high[f] += 1                            
            if f not in conn_priority:                    
                conn_priority[f] = fragment_conn_priority(f, label_weights)  
    ...
    all_frags = set(freq_high) | set(freq_low)            
    N_high, N_low = len(high_mols), len(low_mols)         

    enrich = {}                                           
    for f in all_frags:                                   
        ph = (freq_high[f] + alpha) / (N_high + 2 * alpha)  
        pl = (freq_low[f] + alpha) / (N_low + 2 * alpha)    

        ph = min(max(ph, 1e-6), 1 - 1e-6)                  
        pl = min(max(pl, 1e-6), 1 - 1e-6)                  
        enrich[f] = float(np.log(ph / (1 - ph)) - np.log(pl / (1 - pl)))  

    conn_z = _zscore(conn_priority)                        
    enrich_z = _zscore(enrich)                             

    importance = {}                                        
    for f in all_frags:                                    
        importance[f] = w_conn * conn_z.get(f, 0.0) + w_enrich * enrich_z.get(f, 0.0)  

    return importance    

import numpy as np

def calc_frag_weights(high_frag_freq, low_frag_freq, epsilon=1e-3):
    ...
    for frag in all_frags:
        ...
        weights[frag] = np.log((fh + epsilon) / (fl + epsilon))
    return weights

def structure_reward_for_mol(smiles, frag_weights):
    frags = get_brics_fragments(smiles)
    scores = [frag_weights.get(f, 0.0) for f in frags]
    if len(scores) == 0:
        return 0.0
    return np.mean(scores)

def reward_normalize(raw_reward, scale=2):
    return 5 * np.tanh(raw_reward / scale)

def calc_reward_for_mol_structure(mol, high_mols, low_mols):
    high_frag_freq = count_frag_freq(high_mols)
    low_frag_freq  = count_frag_freq(low_mols)
    frag_weights = calc_frag_weights(high_frag_freq, low_frag_freq, epsilon=1e-3)
    return reward_normalize(structure_reward_for_mol(mol, frag_weights))
