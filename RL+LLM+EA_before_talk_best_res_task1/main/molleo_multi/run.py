                ...
                for _ in range(config["offspring_size"]):
                    ...
                    while attempts < max_retry:
                        ...
                        if off is None or len(off) == 0:
                            ...
                            continue
                        # offspring_mol += off
                        # parent_map[Chem.MolToSmiles(off[0])] = (parents_mols) 
                        ...
                        cand_pairs = list(zip(temp_mols, [float(s) for s in temp_scores], temp_smiles, temp_details))
                        ...
                        if cand_pairs:
                            ...
                            oracle_scores, oracle_details = query_oracle_with_cache(
                                self.oracle,
                                topk_smiles,
                                self.oracle_score_cache,
                                self.oracle_detail_cache
                            ...
                            oracle_ranked = [
                                (surrogate_topk[i][0], oracle_scores[i], surrogate_topk[i][2], oracle_details[i])
                                for i in range(len(surrogate_topk))
                            ]
                            best_off, best_score, best_smi, cand_scor_detaili = max(oracle_ranked, key=lambda x: x[1])
                            print(f"best mol: {best_off, best_score, best_smi, cand_scor_detaili}, temp_flag: {temp_flag}, no_improve_rounds: {no_improve_rounds}")
