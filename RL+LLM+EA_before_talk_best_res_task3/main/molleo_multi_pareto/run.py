...
def update_oracle_local_buffer(smiles_list, score_list, detail_list, score_buffer, detail_buffer):
    for smi, score, detail in zip(smiles_list, score_list, detail_list):
        if smi is None:
            continue
        score_buffer[smi] = score
        detail_buffer[smi] = detail

def query_oracle_with_local_buffer(oracle, smiles_list, score_buffer, detail_buffer=None):
    """Local score buffer is independent from oracle.mol_buffer and clean_buffer()."""
    if detail_buffer is None:
        detail_buffer = {}

    need_query = [smi for smi in smiles_list if smi not in score_buffer]

    if need_query:
        scores, details = oracle(need_query)
        update_oracle_local_buffer(need_query, scores, details, score_buffer, detail_buffer)

    scores_out = [score_buffer[smi] for smi in smiles_list]
    details_out = [detail_buffer.get(smi, None) for smi in smiles_list]
...
## Fragment content:\n
        """
        self.smiles_score_clusters = pd.read_csv(settings.csv_path_url3_pareto)
        ...
        self.init_crossatt_path = settings.model_init_path_url3_pareto
        self.init_crossatt_path_pre = settings.model_init_predictor_path_url3_pareto
        self.add_crossatt_path = settings.model_add_path_url3_pareto
        self.add_crossatt_path_pre = settings.model_add_predictor_path_url3_pareto
        ...
        self.reward_engine = RewardEngine(
            ...
            all_smiles=set(self.smiles_score_clusters['smiles'].astype(str).tolist()), 
        ...
        while True:
            ...
            if self.args.mol_lm == 'GPT-4':
                ...
                if not accepted:
                    ...
                    offspring_mol.append(best_off)
                    parent_map[Chem.MolToSmiles(best_off, canonical=True)] = parents_mols
                    action_records.append({
                        "state": s.detach().cpu(),
                        "action": action,
                        "reward": float(best_score - max(parent_scores)),
            ...
            high_tuples = sorted(high_tuples, key=lambda x: x[0], reverse=True)
            ...
            low_tuples = sorted(low_tuples, key=lambda x: x[0], reverse=True)
            ...
            # population_scores_detail = scores_details_temp
