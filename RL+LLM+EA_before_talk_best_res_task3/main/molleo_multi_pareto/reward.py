...
class RewardEngine:
    ...
    def refresh_memory(
        ...
        if all_smiles is not None:
            self.all_smiles |= set(all_smiles)

        if recompute_frag_weights:
            high_ff = count_frag_freq(self.high_mols or [])
            low_ff = count_frag_freq(self.low_mols or [])
            self.frag_weights = calc_frag_weights(high_ff, low_ff, epsilon=1e-3)
        return self
    ...
    def get_reward(self,
        ...
        for mol, main in zip(mol_list, main_scores):
            ...
            # main score in this codebase is already a maximization target
            # (higher oracle score means better molecule), so keep the signal
            # monotone instead of converting it into a distance from 0.9.
            main_raw = float(main)
            tox_raw = 0.0
    ...
    def score_mol(self, mol) -> Dict[str, Any]:
        ...
        for name, catalog in self.catalogs.items():
            ...
            if entries:
                ...
                raw += self.weights.get(name, 0.0) * len(entries)  
