...
def top_auc(buffer, top_n, finish, freq_log, max_oracle_calls):
    ...
    prev = 0
    called = 0
    ordered_results = list(sorted(buffer.items(), key=lambda kv: kv[1][2], reverse=False)) # increasing order
...
class Oracle:
    ...
    def evaluate(self, smi):
        ...
        for eva in self.max_evaluator:
            ...
            score_list.append(f"{eva.name}: {temp_score}")
        for eva in self.min_evaluator:
            temp_score = eva(smi)
            if eva.name == 'sa':
                score = score + (1 - ((temp_score - 1)/9))
                score_list.append(f"{eva.name}: {temp_score};")
            else:
                score = score + (1 - temp_score)
                score_list.append(f"{eva.name}: {temp_score};")
    ...
    def __call__(self, smiles_lst):
        ...
        if type(smiles_lst) == list:
            ...
            for smi in smiles_lst:
                eval_score, eval_score_detail = self.score_smi(smi)
                score_list.append(eval_score)
                score_list_detail.append(eval_score_detail)
            ...
            score_list, score_list_detail = self.score_smi(smiles_lst)
            if len(self.mol_buffer) % self.freq_log == 0 and len(self.mol_buffer) > self.last_log:
                self.sort_buffer()
                self.log_intermediate()
                self.last_log = len(self.mol_buffer)
                self.save_result(self.task_label)
        return score_list, score_list_detail
...
class BaseOptimizer:
    ...
    def sort_buffer_other(self):
        self.oracle_other.sort_buffer()

    def log_intermediate(self, mols=None, scores=None, finish=False):
        self.oracle.log_intermediate(mols=mols, scores=scores, finish=finish)

    def log_intermediate_other(self, mols=None, scores=None, finish=False):
        self.oracle_other.log_intermediate(mols=mols, scores=scores, finish=finish)
