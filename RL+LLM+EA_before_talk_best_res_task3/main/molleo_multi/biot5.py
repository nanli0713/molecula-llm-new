...
class BioT5:
        ...
        if isinstance(block, (list, tuple)):
            ...
            for item in block:
                if isinstance(item, dict):
                    ...
                    lines.append(f"{header} [Mol FRAG] {frag} | attachment points={labels} | cut times={n_cuts}")
    ...
    def edit(self, smiles_list, h_fragments=None, l_fragments=None,  past_generation=None, past_generation_total=None, action=2):
        ...
        for i, MOL in enumerate(smiles_list):
            ...
            h_block = self._format_fragment_block(h_fragments, "High score fragments:")
            l_block = self._format_fragment_block(l_fragments, "Low score fragments:")
            if h_block and action == 0:
                task_input += self.good_cases1 + h_block + self.good_cases2
            elif l_block and action == 1:
                task_input += self.bad_cases1 + l_block + self.bad_cases2
            elif h_block and l_block and action == 2:
                task_input += self.good_cases1 + h_block + self.good_cases2 + self.bad_cases1 + l_block + self.bad_cases2
