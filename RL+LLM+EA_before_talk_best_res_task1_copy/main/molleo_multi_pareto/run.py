...
def make_mating_pool(population_mol: List[Mol], population_scores, population_scores_detail, offspring_size: int):
    ...
    """
    # scores -> probs
    print(f"length of them: {len(population_scores)}, {len(population_mol)}, {len(population_scores_detail)}")
    all_tuples = list(zip(population_scores, population_mol, population_scores_detail))
    population_scores = [s + MINIMUM for s in population_scores]
    sum_scores = sum(population_scores)
    population_probs = [p / sum_scores for p in population_scores]
    print(f"length of them: {len(all_tuples)}, {len(population_probs)}")
...
## Fragment content:\n
        """
        self.smiles_score_clusters = pd.read_csv(settings.csv_path_url)
        ...
        self.high_score_mols = self.smiles_score_clusters.sort_values(by='scores', ascending=False).head(int(len(self.smiles_score_clusters)*0.2)).reset_index(drop=True)['smiles'].tolist()
        self.low_score_mols = self.smiles_score_clusters.sort_values(by='scores', ascending=False).tail(int(len(self.smiles_score_clusters)*0.2)).reset_index(drop=True)['smiles'].tolist()
        ...
        self.high_scores = self.smiles_score_clusters.sort_values(by='scores', ascending=False).head(int(len(self.smiles_score_clusters)*0.2)).reset_index(drop=True)['scores'].tolist()
        self.low_scores = self.smiles_score_clusters.sort_values(by='scores', ascending=False).tail(int(len(self.smiles_score_clusters)*0.2)).reset_index(drop=True)['scores'].tolist()
    ...
    def _optimize(self, config):
        ...
        population_mol = [mol for mol in population_mol if mol is not None]

        population_smiles = [Chem.MolToSmiles(mol, canonical=True) for mol in population_mol]
        population_scores, population_scores_detail = self.oracle(population_smiles)

        for _smi, _score, _detail in zip(population_smiles, population_scores, population_scores_detail):
            self.oracle_score_cache[_smi] = _score
            self.oracle_detail_cache[_smi] = _detail

        print(f"Length of population_mol:{len(population_mol)}, {len(population_scores_detail)}")
        ...
        while True:
            ...
            mating_tuples = make_mating_pool(
                population_mol,
                population_scores,
                population_scores_detail,
            ...
            if self.args.mol_lm == 'GPT-4':
                ...
                offspring_mol = [
                    reproduce(mating_tuples, config["mutation_rate"])
                    for _ in range(config["offspring_size"])
                ]
                ...
                while len(editted_smi) < self.args.bin_size:
                    ...
                    while attempts < max_retry and not accepted:
                        ...
                        if edited_mols is None or len(edited_mols) == 0:
                            print(f"Attempt {attempts}: No valid candidates from edit!")
                            continue

                        edited_mols = edited_mols[0]

                        if not edited_mols:
                            print("No new mol generated!")
                            continue

                        edited_mols = [m for m in edited_mols if m is not None]

                        if len(edited_mols) == 0:
                            print("No new mol generated!")
                            continue

                        edited_smiles = [
                            Chem.MolToSmiles(m, canonical=True)
                            for m in edited_mols
                            if m is not None
                        ]
                        ...
                        best_mol, best_score, best_smi, best_detail = max(
                            cand_pairs,
                            key=lambda x: x[1]
                        )

                        if best_score > parent_scores:
                            print(
                                f"Accepted supplement: {best_score:.3f} > "
                                f"parent {parent_scores:.3f}, SMILES={best_smi}"
                            )
                ...
                offspring_mol = [m for m in offspring_mol if m is not None]
            ...
            print(f"before pareto population_mol: {len(population_mol)}")
            population_mol = self.oracle.select_pareto_front(
                [Chem.MolToSmiles(mol) for mol in population_mol]
            )
            print(f"after pareto population_mol: {len(population_mol)}")
            ...
            reward_scores = self.reward_engine.get_reward(
                ...
                parent_map,
                parent_main_scores,
                np.mean(sorted(population_scores)[:10])
            )

            print(
                f"population_scores: {len(population_scores)}, "
                f"offspring_mol:{len(offspring_mol)}, "
                f"valid_offspring:{len(valid_offspring_mol)}"
            )
                ...
                if len(offspring_mol_temp) == 0:
                    ...
                    with CrossAttTrainLock(_crossatt_lock_label(self.args)):
                        if iteration != 1 and iteration != 2 and iteration != 3:
                            self.cross_att.load_state_dict(
                                torch.load(self.add_crossatt_path, map_location=self.device)
                            )
                            self.cross_att_predictor.load_state_dict(
                                torch.load(self.add_crossatt_path_pre, map_location=self.device)
                            )
                        ...
                        lora_wrapped = _replace_linear_with_lora(
                            att_self_module,
                            r=4,
                            alpha=8,
                            dropout=0.05
                        ...
                        training_batch = offspring_mol_temp
                        ...
                        for item in training_batch:
                            try:
                                smiles_batch.append(Chem.MolToSmiles(item[0], canonical=True))
                        ...
                        if len(smiles_batch) > 0:
                            ...
                            inc_optimizer = torch.optim.AdamW(
                                trainable_params,
                                lr=2e-4,
                                weight_decay=1e-4
                            )
                            ...
                            for epoch in tqdm(range(5)):
                                ...
                                for batch in incremental_loader:
                                    batch = {
                                        k: v.to(device=self.device) if isinstance(v, torch.Tensor) else v
                                        for k, v in batch.items()
                                    }
            ...
            candidates = [
                (parent_mol[2], parent_score[2]),
                (parent_mol[3], parent_score[3])
            ]
            ...
            for _m, _s in zip(self.high_score_mols, self.high_scores):
                try:
                    high_pool_mols.append(ensure_mol(_m))
                    high_pool_scores.append(_s)
                except Exception:
                    continue
            ...
            for _m, _s in zip(self.low_score_mols, self.low_scores):
                try:
                    low_pool_mols.append(ensure_mol(_m))
                    low_pool_scores.append(_s)
                except Exception:
                    continue
            ...
            for _score, _smi, _detail in _high_top3:
                try:
                    _record = format_generation_record(_smi, _score, _detail)
            ...
            for _score, _smi, _detail in _low_worst3:
                try:
                    _record = format_generation_record(_smi, _score, _detail)
            ...
            s2 = torch.cat(
                [
                    next_state["parent_fp"],
                    next_state["cluster_center"],
                    next_state["high_pool"],
                    next_state["low_pool"]
                ],
                dim=0
            ).to(self.device)
            ...
            self.replay.push(
                s.detach().cpu(),
                action,
                reward_scores[-config["offspring_size"]:],
                None if done else s2.detach().cpu(),
                done
            )
            ...
            population_scores_detail = [t[2] for t in population_tuples]
