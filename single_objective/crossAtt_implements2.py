...
class Graph_encoder(SGATT):
    ...
    def __init__(self, node_features_1, edge_features_1,            
        ...
        for _ in range(edge_features_1):
            self.msg_nns_1.append(
                FeedForwardNetwork(node_features_1,
                                   [msg_hidden_dim] * msg_depth,
                                   message_size,
                                   dropout_p=self.droupout,
                                   bias=False))
        ...
        self.gru_1 = nn.GRUCell(input_size=message_size,
                                hidden_size=node_features_1,
                                bias=False)
        ...
        out_layer_sizes = [
            round(out_hidden_dim * (out_layer_shrinkage ** (i / (out_depth - 1 + 1e-9))))
            for i in range(out_depth)
        ] 

        self.out_nn = FeedForwardNetwork(
            gather_width * 2, out_layer_sizes, out_features,
            dropout_p=self.droupout)
    ...
    def aggregate_message_1(self, nodes, node_neighbours, edges, node_neighbour_mask):
        ...
        embeddings_masked_per_edge = [
            edges[:, :, i].unsqueeze(-1) * self.msg_nns_1[i](node_neighbours)
            for i in range(self.edge_fts_1)
        ]
        embedding = sum(embeddings_masked_per_edge)

        energies_masked_per_edge = [
            edges[:, :, i].unsqueeze(-1) * self.att_nns_1[i](node_neighbours)
            for i in range(self.edge_fts_1)
        ]
    ...
    def readout(self, input_nodes, node_mask):
        ...
        return torch.from_numpy(graph_embeddings)
