...
class SGATT(nn.Module):
    def __init__(self, node_fts_1, edge_fts_1,
                 message_size, message_passes, out_fts, kg_path):
    ...
    def forward(self, mol, adj_1, nd_1, ed_1, de_1, mask_1,
        ...
        # 先合并为一个 numpy 数组
        fp_array = np.array([self.smile2fp(m) for m in mol])
        # 再一次性转为 tensor
        morgan_fp = torch.from_numpy(fp_array).to(device)
...
def compute_mol_features(smiles, max_seq_len=50, kg_pickle_path=""):
