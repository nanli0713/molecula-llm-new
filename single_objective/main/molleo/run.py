...
def load_starting_population_from_csv(
    csv_path,
    population_size=None,
    smiles_column="smiles",
    score_column="jnk3_score",
    sort_by_score=True,
    canonical=True,
):
    """
    从 CSV 文件中读取初始种群。默认读取 eval_jnk3.py 输出的 smiles 列。
    ...
    if smiles_column not in df.columns:
        raise ValueError(
            f"Column '{smiles_column}' not found in CSV: {csv_path}. "
    ...
    for _, row in df.iterrows():
        ...
        if mol is None:
            print(f"[CSV init] invalid SMILES skipped: {smi}")
            continue
        ...
        score = _as_float_or_none(row.get(score_column)) if has_score else None
        records.append((smi_can, score))

    if len(records) == 0:
        raise ValueError(f"No valid SMILES found in CSV: {csv_path}")

    # 去重：如果 canonical SMILES 重复，保留最高分的那个
    best = {}
    for smi, score in records:
        if smi not in best:
            best[smi] = score
        else:
            old_score = best[smi]
    ...
    # CSV 是 top120 结果时本身已经按 jnk3_score 降序；这里保留排序以防后续文件顺序变化
    if sort_by_score and has_score:
        records.sort(
    ...
    if population_size is not None:
        records = records[:population_size]

    starting_population = [smi for smi, _ in records]
    score_cache = {
        smi: score
        for smi, score in records
    ...
    print(f"[CSV init] loaded {len(starting_population)} molecules from {csv_path}")

    return starting_population, score_cache
...
class GB_GA_Optimizer(BaseOptimizer):
    ...
    def __init__(self, args=None):
        ...
        weight_path1 = '/root/autodl-tmp/single_objective/weights' + f'/{self.args.oracles[0]}_scores_graph_encoder.pth'
        weight_path2 = '/root/autodl-tmp/single_objective/weights' + f'/{self.args.oracles[0]}_scores_predictor.pth'
        weight_path3 = '/root/autodl-tmp/single_objective/add_weights' + f'/add_{self.args.oracles[0]}_scores_graph_encoder.pth'
        weight_path4 = '/root/autodl-tmp/single_objective/add_weights' + f'/add_{self.args.oracles[0]}_scores_predictor.pth'
        data_path = '/root/autodl-tmp/single_objective/data' + f'/selected_molecules_{self.args.oracles[0]}_scores.csv'
...
## Fragment content:\n
    ...
    def _optimize(self, oracle, config):
        ...
        # csv_start_path = "/root/autodl-tmp/top120_jnk3.csv"

        # use_csv_scores_as_cache = False
        # # 默认只使用 CSV 的 smiles 列初始化种群，后续仍由当前 oracle 重新打分。
        # # 如果要直接复用 CSV 中的 jnk3_score，可改为 True。

        # if os.path.exists(csv_start_path):
        #     starting_population, csv_score_cache = load_starting_population_from_csv(
        #         csv_start_path,
        ...
        #     # 如果 CSV 分子数不足 population_size，则用 self.all_smiles 补齐
        #     if len(starting_population) < config["population_size"]:
        #         need = config["population_size"] - len(starting_population)
        #         existing = set(starting_population)

        #         supplement = []
        #         candidate_smiles = list(self.all_smiles)
        #         random.shuffle(candidate_smiles)
        ...
        #             smi_can = Chem.MolToSmiles(mol, canonical=True)
        #             if smi_can in existing:
        #                 continue

        #             supplement.append(smi_can)
        #             existing.add(smi_can)

        #             if len(supplement) >= need:
        #                 break

        #         starting_population += supplement

        #     if len(starting_population) < config["population_size"]:
        #         raise ValueError(
        #             f"Only got {len(starting_population)} valid starting molecules, "
        #             f"but population_size={config['population_size']}"
        #         )

        #     # 是否直接复用 CSV 里的 score
        #     if use_csv_scores_as_cache:
        #         self.oracle_score_cache.update(csv_score_cache)
        #         print(f"[CSV init] loaded {len(csv_score_cache)} scores into oracle_score_cache")

        # else:
        #     print(f"[CSV init] file not found: {csv_start_path}")
        #     print("[CSV init] fallback to original initialization")
        ...
        while True:
            ...
            if self.args.mol_lm == "GPT-4":
                for _ in range(config["offspring_size"]):
                    ...
                    while attempts < max_retry and not accepted:
                        ...
                        if cand_pairs:
                            ...
                            oracle_ranked = [
                                (selected_pairs[i][0], oracle_scores[i], selected_pairs[i][2])
                                for i in range(len(selected_pairs))
                            ]
                            best_off, best_score, best_smi = max(oracle_ranked, key=lambda x: x[1])
                            print(f"best mol: {best_off, best_score, best_smi}, selected_for_oracle: {len(oracle_ranked)}, no_improve_rounds: {no_improve_rounds}")
                ...
                while len(editted_smi) < self.args.bin_size:
                    ...
                    # 每一轮都重新生成 action，让判断逻辑更接近 GPT-4 的逐轮决策
                    action_parents = [parent_mols, parent_mols]
                    state = build_state_dict(
                    ...
                    cid = nearest_cluster_id(action_parents[0], action_parents[1], self.cluster_centers, fp_type="morgan")
                    with torch.no_grad():
                        q_vals = self.trainer.q(s.unsqueeze(0))
                    action = self.selector.pick(q_vals, cluster_id=cid)
                    last_state_tensor = s
                    last_action = action
                    last_next_parents = action_parents
                    ...
                    while attempts < max_retry and not accepted:
                        ...
                        if best_score > parent_scores:  # 严格优于父代
                            ...
                            accepted = True
                            parent_map[best_smi] = action_parents
