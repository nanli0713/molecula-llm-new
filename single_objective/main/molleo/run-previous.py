...
MINIMUM = 1e-10
DEFAULT_CLUSTER_CENTER_FPS = None
...
def nearest_cluster_id(
    ...
    fp_type="morgan",
    weights=(0.5, 0.5),
    cluster_centers_fps=None,
    cluster_ids=None,
):
    centers_fps = cluster_centers_fps if cluster_centers_fps is not None else DEFAULT_CLUSTER_CENTER_FPS
    cluster_ids = cluster_ids if cluster_ids is not None else DEFAULT_CLUSTER_CENTER_IDS

    if centers_fps is None:
        centers_fps = []
        cluster_ids = []
    ...
    if cluster_ids is None:
        cluster_ids = list(range(len(centers_fps)))
...
def get_cluster_center_feature(current_mol, clustering_data, fp_type="morgan", cluster_centers_fps=None):
    centers_fps = cluster_centers_fps if cluster_centers_fps is not None else DEFAULT_CLUSTER_CENTER_FPS
    if centers_fps is None:
        centers_fps = []
    ...
    sims = DataStructs.BulkTanimotoSimilarity(fp_current, centers_fps)
    best_idx = int(np.argmax(sims)) 
    center_fp = centers_fps[best_idx] 
...
def build_state_dict(
    ...
    fp_type="morgan",
    cluster_centers_fps=None,
    high_pool_feature=None,
    low_pool_feature=None,
):
    if type(parents[0]) is not rdkit.Chem.rdchem.Mol: 
        print("run build_state_dict mols:", parents[0])

    if cluster_centers_fps is None:
        cluster_centers_fps = DEFAULT_CLUSTER_CENTER_FPS
    if high_pool_feature is None:
        high_pool_feature = CURRENT_HIGH_POOL_FEATURE
    if low_pool_feature is None:
        low_pool_feature = CURRENT_LOW_POOL_FEATURE
    if high_pool_feature is None:
        high_pool_feature = get_mol_pool_feature(high_score_mols, fp_type)
    if low_pool_feature is None:
        low_pool_feature = get_mol_pool_feature(low_score_mols, fp_type)
    ...
    state = {
        ...
        "cluster_center": get_cluster_center_feature(
            parents[0], clustering_data, fp_type, cluster_centers_fps=cluster_centers_fps
        ) + get_cluster_center_feature(
            parents[1], clustering_data, fp_type, cluster_centers_fps=cluster_centers_fps
        ),

        "high_pool": high_pool_feature,
        "low_pool": low_pool_feature,
    }
    return state
...
def query_LLM(question, model="gpt-5.2", temperature=0.5):
...
def query_oracle_with_cache(oracle, smiles_list, score_cache, detail_cache=None):
    ...
    if need_query:
        scores = oracle(need_query)
        for smi, s in zip(need_query, scores):
            score_cache[smi] = s
...
class GB_GA_Optimizer(BaseOptimizer):
    ...
    def __init__(self, args=None):
        ...
        if args.mol_lm == "GPT-4":
            self.mol_lm = GPT4(api_key=getattr(args, "key", None), base_url=settings.base_url)
        ...
        weight_path1 = '/root/autodl-tmp/yinan/single_objective/weights' + f'/{self.args.oracles[0]}_scores_graph_encoder.pth'
        weight_path2 = '/root/autodl-tmp/yinan/single_objective/weights' + f'/{self.args.oracles[0]}_scores_predictor.pth'
        weight_path3 = '/root/autodl-tmp/yinan/single_objective/add_weights' + f'/add_{self.args.oracles[0]}_scores_graph_encoder.pth'
        weight_path4 = '/root/autodl-tmp/yinan/single_objective/add_weights' + f'/add_{self.args.oracles[0]}_scores_predictor.pth'
        data_path = '/root/autodl-tmp/yinan/single_objective/data' + f'/selected_molecules_{self.args.oracles[0]}_scores.csv'
        self.task_desc1_high = f"Please read the following descriptions of high-scoring molecular fragments and summarize each fragment based on its structural characteristics (e.g., aromatic ring type, linkage, substituents, heteroatom distribution, etc.).\nThe summary must be within 60 words. \n\n## For each fragment:\n\n- Describe its main structural features concisely.\n\n## Fragment content:\n\n"
...
Do not omit any important high-score details, especially attachment point information. Explanation should summarize the disconnected position and surrounding chemical features based on the attachment point information. Explanation not more than 10 words. And no Line Break in this block.
## Fragment content:\n
        """
...
Do not omit any important low-score details, especially attachment point information. Explanation should summarize where the fragment was disconnected and the nearby chemical features based on the attachment point information. Explanation not more than 10 words. And no Line Break in this block.

## Fragment content:\n
        """
        self.smiles_score_clusters = pd.read_csv(data_path)
        ...
        self.init_crossatt_path = weight_path1
        self.init_crossatt_path_pre = weight_path2
        self.add_crossatt_path = weight_path3
        self.add_crossatt_path_pre = weight_path4
        ...
        global DEFAULT_CLUSTER_CENTER_FPS, DEFAULT_CLUSTER_CENTER_IDS
        self.cluster_centers = []
        self.cluster_centers_fps = []
        self.cluster_center_ids = []
        for cid, cluster in enumerate(self.clustering_data[1]):
            if len(cluster) == 0:
                continue
            center_mol, center_fp = compute_center_mol(cluster, fp_type="morgan")
            self.cluster_centers.append(center_mol)
            self.cluster_center_ids.append(cid)
            self.cluster_centers_fps.append(center_fp)
        DEFAULT_CLUSTER_CENTER_FPS = self.cluster_centers_fps
        DEFAULT_CLUSTER_CENTER_IDS = self.cluster_center_ids
    ...
    #     import os
    #     print("CWD:", os.getcwd())
    #     print("gsk3b.pkl exists:", os.path.exists("/home/lachesis/agent-chat/MOLLEO-main-server2/single_objective/oracle/gsk3b.pkl"))
    #     print("gsk3b_current.pkl exists:", os.path.exists("/home/lachesis/agent-chat/MOLLEO-main-server2/single_objective/oracle/gsk3b_current.pkl"))
    #     print("jnk3.pkl exists:", os.path.exists("/home/lachesis/agent-chat/MOLLEO-main-server2/single_objective/oracle/jnk3.pkl"))
    #     print("jnk3_current.pkl exists:", os.path.exists("/home/lachesis/agent-chat/MOLLEO-main-server2/single_objective/oracle/jnk3_current.pkl"))
    ...
    #     population_scores = query_oracle_with_cache(
    #         self.oracle,
    #         population_smiles,
    #         self.oracle_score_cache
    #     )
    ...
    #             for line in h_prompt_lines:
    #                 if "FRAG]" not in line:
    #                     continue
    #                 h_mols.append(line.split("FRAG]")[1].split("|")[0].strip())
    #             combine_prompt_high = "\n".join(h_mols[:5])
    #             l_mols = []
    #             for line in l_prompt_lines:
    #                 if "FRAG]" not in line:
    #                     continue
    #                 l_mols.append(line.split("FRAG]")[1].split("|")[0].strip())
    #             combine_prompt_low = "\n".join(l_mols[:5])
    ...
    #             clustering_data=self.cluster_centers,
    ...
    #         cid = nearest_cluster_id(parents_mols[0], parents_mols[1], self.cluster_centers, fp_type="morgan")
    #         with torch.no_grad():
    ...
    #                         parents_mols, parent_scores, None,
    ...
    #                         print("no valid candidates!")
    #                         continue
    #                     off_scores = self.surrogate_score(off)
    #                     score_details = ["surrogate"] * len(off_scores)
    #                     temp_mols += off
    #                     temp_smiles += [Chem.MolToSmiles(item) for item in off]
    #                     temp_scores += off_scores
    #                     temp_details += score_details
                        
    #                     cand_pairs = list(zip(temp_mols, [float(s) for s in temp_scores], temp_smiles))
    #                     # best_off, best_score, best_smi, cand_scor_detaili = max(cand_pairs, key=lambda x: x[1])
    #                     if cand_pairs:
    #                         # temp_flag = min(no_improve_rounds // 2 + 1, 2)
    #                         temp_flag = 2
    #                         topk = min(temp_flag, len(cand_pairs))

    #                         surrogate_topk = sorted(cand_pairs, key=lambda x: x[1], reverse=True)[:topk]
    #                         topk_smiles = [item[2] for item in surrogate_topk]

    #                         # oracle_scores, oracle_details = self.oracle(topk_smiles)
    #                         oracle_scores = query_oracle_with_cache(
    #                             self.oracle,
    #                             topk_smiles,
    #                             self.oracle_score_cache
    #                         )
    ...
    #                         oracle_ranked = [
    #                             (surrogate_topk[i][0], oracle_scores[i], surrogate_topk[i][2])
    #                             for i in range(len(surrogate_topk))
    #                         ]
    #                         best_off, best_score, best_smi = max(oracle_ranked, key=lambda x: x[1])
    #                         print(f"best mol: {best_off, best_score, best_smi}, temp_flag: {temp_flag}, no_improve_rounds: {no_improve_rounds}")
    #                         if best_score <= min(parent_scores):
    #                             low_record = format_generation_record(best_smi, best_score, None)
    ...
    #                     best_off, best_score, best_smi = best_oracle_candidate
    #                     offspring_mol.append(best_off)
    #                     parent_map[Chem.MolToSmiles(best_off)] = parents_mols
    ...
    #         if self.args.mol_lm == 'GPT-4':    
    #             parent_main_scores = {
    #                 parent_mol[0]: parent_scores[0],  
    #                 parent_mol[1]: parent_scores[1],  
    #             }
    #         elif self.args.mol_lm == 'BioT5':
    #             parent_main_scores = {
    ...
    #         reward_scores = self.reward_engine.get_reward(population_mol, population_scores, toxic_out["probs"], parent_map, parent_main_scores, np.mean(sorted(population_scores)[:10]))
    #         print(f"population_scores: {len(population_scores)}, offspring_mol:{len(offspring_mol)}")
    #         if iteration % 3 == 0:
    ...
    #         done = self.episode.step(reward_scores[-config["offspring_size"]:])
    #         self.replay.push(s.detach().cpu(), action, reward_scores[-config["offspring_size"]:], (None if done else s2.detach().cpu()), done)
    #         if done:
    #             self.episode.reset()
    ...
    #         low_tuples = sorted(low_tuples, key=lambda x: x[0], reverse=False)[:len(self.high_score_mols)]
    ...
    def _optimize(self, oracle, config):
        ...
        while True:
            ...
            if self.args.mol_lm == "GPT-4":
               for _ in range(config["offspring_size"]):
                    ...
                    while attempts < max_retry:
                        ...
                        off = [m for m in off if m is not None]
                        ...
                        for _m in off:
                            try:
                                _smi = Chem.MolToSmiles(_m, canonical=True)
                                parent_map[_smi] = parents_mols
                            except Exception:
                                continue  
            elif self.args.mol_lm == "BioT5":
                # ===== 新增：初始化全局历史记录（保留Top 2）=====
            ...
            # ============================================================
            # Build past_generation_total / past_generation_total_low
            # strictly from current population, not from historical pools.
            # This avoids stale high_scores / low_scores contaminating prompt.
            # ============================================================
            ...
            for _idx, (_score, _mol) in enumerate(zip(population_scores, population_mol)):
                try:
                    if _mol is None:
                        continue

                    if isinstance(_mol, str):
                        _tmp_mol = Chem.MolFromSmiles(_mol)
                        if _tmp_mol is None:
                            continue
                        _smi = Chem.MolToSmiles(_tmp_mol, canonical=True)
                    else:
                        _smi = Chem.MolToSmiles(_mol, canonical=True)

                    # if _idx < len(population_scores_detail):
                    #     _detail = population_scores_detail[_idx]
                    # else:
                    #     _detail = self.oracle_detail_cache.get(_smi, None)

                    current_population_records.append(
                        (_score, _smi)
                    )

                except Exception as e:
                    print(f"Skip invalid population mol when building past_generation records: {e}")
                    continue

            # Top-3 from current population only
            _high_top3 = sorted(
                current_population_records,
                key=lambda x: x[0],
                reverse=True
            ...
            for _score, _smi in _high_top3:
                try:
                    _record = format_generation_record(_smi, _score, None)
                    past_generation_total.append(_record)
                except Exception as e:
                    print(f"Failed to format high generation record: {e}")
                    continue

            # Worst-3 from current population only
            _low_worst3 = sorted(
                current_population_records,
                key=lambda x: x[0],
                reverse=False
            ...
            for _score, _smi in _low_worst3:
                try:
                    _record = format_generation_record(_smi, _score, None)
                    past_generation_total_low.append(_record)
                except Exception as e:
                    print(f"Failed to format low generation record: {e}")
                    continue

            print("Updated past_generation_total Top-3 from current population:")
            for item in past_generation_total:
                print(item)

            print("Updated past_generation_total_low Worst-3 from current population:")
            for item in past_generation_total_low:
                print(item)
            next_state = build_state_dict(
                parents=next_parents,
                clustering_data=self.cluster_centers,
